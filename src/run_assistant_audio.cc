...
static const char kDeviceModelId[] = "myassistant-fd356-myassistant-0zwd5h";
...
std::shared_ptr<Channel> createChannel(const std::string& host)
{
	...
	if (verbose)
	{
		std::clog << "assistant_sdk CreateCustomChannel(" << server << ", creds, arg)" << std::endl
		          << std::endl;
	}
                         ...
                         std::string* html_out_command)
{
	...
	*api_endpoint = ASSISTANT_ENDPOINT;
	...
	{
		int option_index;
		int option_char = getopt_long(argc, argv, "c:e:l:v:h", long_options, &option_index);
		if (option_char == -1)
		{
			break;
		}
...
int main(int argc, char** argv)
{
...
#ifndef ENABLE_ALSA
	std::cerr << "ALSA audio input is not supported on this platform." << std::endl;
	return -1;
#endif
	...
	grpc_init();
	if (!GetCommandLineFlags(argc, argv, &credentials_file_path, &api_endpoint, &locale,
	                         &html_out_command))
	{
		return -1;
	}
	...
	while (true)
	{
		...
		if (!credentials_file)
		{
			std::cerr << "Credentials file \"" << credentials_file_path << "\" does not exist."
			          << std::endl;
			return -1;
		}
		...
		if (verbose)
		{
			std::clog << "assistant_sdk wrote first request: " << request.ShortDebugString()
			          << std::endl;
		}
		stream->Write(request);

		audio_input = std::make_unique<AudioInputALSA>();
		...
		{ // Returns false when no more to read.
			...
			if (!html_out_command.empty() && !response.screen_out().data().empty())
			{
				std::string html_out_base64 = base64_encode(response.screen_out().data());
				system(
				    (html_out_command + " \"data:text/html;base64, " + html_out_base64 + "\"").c_str());
			}
